---
name: prove-missing
description: Look up a theorem by ID, write a complete informal proof from Claude's own mathematical knowledge, then formalize it with lean4:prove.
---
...
> or permission at any step — including subskill invocations (lean4:prove, lean4:autoprove,
...
8. `site:optpku.github.io/ReasBook "<theorem_name>"` — check ReasBook for a proof or formalization

Also **check optlib locally** before relying on WebSearch:
Read `/Users/apple/.claude/skills/optlib-nav/references/optlib-index.md` and search for
`<theorem_name>` or `<key_lemma>` in the index. The index lists all major declarations
per module — if a match is found, use only that specific import (do NOT read the full
source file unless you need the exact signature). Skip to Step 6 if optlib already has it.
...
For **each step** in the informal proof `_informal.md`:

**Run all three searches in parallel** (single message, three tool calls):
1. `lean_leansearch` — key mathematical claim; record top-3 results
2. `lean_loogle` — type pattern of the key lemma needed; record top-3 results
3. `lean_leanfinder` — description of what the step proves; record top-2 results

Collect results into a "Per-step lemma map":
```
Step N: <title>
...
For steps where NO candidate is found after all 3 searches: mark as
...
## Informal proof
Read the full contents of this file: $INFORMAL_FILE
Do NOT skip any step — it contains the complete per-step tactic sketch and lemma map.
