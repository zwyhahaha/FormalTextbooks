---
name: prove-theorem
description: Look up and autoprove a theorem by ID from the project's index, then update status and log.
---
...
Set `book` = the path component between `proofs/` and the filename in `lean_file`
(e.g. `lean_file = "proofs/Bubeck_convex_optimization/Lemma23.lean"` → `book = "Bubeck_convex_optimization"`).

### 2. Build

Run:
```bash
lake exe cache get && lake build 2>&1 | tail -30
```
...
1. Search for relevant lemmas before writing anything. Check in this order:
   ...
   b. Optlib — read `/Users/apple/.claude/skills/optlib-nav/references/optlib-index.md`
      to find the right module and declaration name (DO NOT read optlib source files directly
      unless the index points you to a specific signature you need). The index has a decision
      tree at the bottom — use it before opening any .lean file.
...
**Phase A — run these three MCP searches in parallel** (single message, three tool calls):
...
**Phase B — run these two bash searches sequentially** (after Phase A):
4. Optlib lookup — consult `/Users/apple/.claude/skills/optlib-nav/references/optlib-index.md`
   using the goal's key terms. Only read the specific source file if the index identifies
   a likely match and you need the exact signature.
...
For each step in the informal proof, run all three searches **in parallel**
...
## Informal proof
Read the full contents of this file: proofs/<book>/informal/<TheoremName>_informal.md
Do NOT skip any step — the file contains the complete per-step tactic sketch and lemma map.
...
### 6. Post-proof (only if the proof finishes with status `proved` — from any tier)
